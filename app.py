...
def grow():
    cursor.execute('SELECT * from ers where email=%s',[uname])
    result=cursor.fetchall()
...
@app.route('/checkstar',methods=['GET'])
def stars():
    ...
    for i in result2:
        print(result2)
        cursor.execute("INSERT INTO ers (email,item,image,price) VALUES(%s,%s,%s,%s)", [result2[i][1],result2[i][2],result2[i][3],result2[i][4]])
...
@app.route('/dropstars', methods=['GET'])
def dropstars():
    item = request.args.get('item')
    ...
    cursor.execute('DELETE from ers * WHERE item=%s',[item])
    cursor.execute('SELECT * from ers WHERE email=%s',[uname])
    result=cursor.fetchall()
   

    
    return render_template("grow.html", length=len(result), result = result)
